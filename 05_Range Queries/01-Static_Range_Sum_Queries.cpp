...
long long getSum(long long segTree[], int i, int lb, int rb, int l, int r)
{
	if(lb==l && rb==r) return segTree[i];
	if(l > r) return 0;
	int mid = (lb+rb)/2;
	return getSum(segTree, 2*i+1, lb, mid, l, min(mid,r)) + getSum(segTree, 2*i+2, mid+1, rb, max(mid+1,l), r);
}
int main()
{
	fastio()
	...
	{
		int l, r; cin >> l >> r;	
		cout << getSum(segTree, 0, 0, n-1, l-1, r-1) << nn;
	}
